...
from rtmag.process.nf2.nf2_data_loader import NpzDataModule, NpzCongridDataModule
...
def main(args):
    ...
    if args.data["type"] == "npz":
        data_module = NpzDataModule(**args.data)
    elif args.data["type"] == "npz_congrid":
        data_module = NpzCongridDataModule(**args.data)
    else:
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=base_path, every_n_train_steps=args.training["validation_interval"], save_last=True
    )

    n_gpus = torch.cuda.device_count()
    trainer = Trainer(max_epochs=args.training['max_epochs'],
                    logger=wandb_logger,
    ...
    with open(os.path.join(base_path, 'runtime.txt'), 'w') as f:
